...
describe("rly-cli", () => {
  ...
  const metadatauri =
    " https://arweave.net:443/fRgc2NeIGpeRcyHQ_cmJE-76AW7jYnTn8kPbiEMXOB ";
  const decimals = 9;
  let tbcPubKey;
  ...
  before(async () => {
    ...
    await tokenB.mintTo(
      ...
      payer,
      [],
      initialTokenBLiquidity.toNumber()
    );
    await tokenA.mintTo(
      callerTokenAAccount,
      payer,
  ...
  it("should create a new token", () => {
    const result = test(
      "create-token",
      `-k ${process.env.KEYPAIR_DEVNET} -n "TestCoin" -s "TC" -d 9 -u ${metadatauri} --supply 100000`
    );
  ...
  it("should add metadata to an existing token", async () => {
    ...
    const result = test(
      "add-metadata",
      `${tokenA.publicKey.toBase58()} -k ${
        process.env.KEYPAIR_DEVNET
      } -n "TestCoin" -s "TC" -u ${metadatauri}`
    );
    const resString = result.trim().split(" ")[0];
  ...
  it("it should freeze a token account", async () => {
    ...
    const result = test(
      ...
      `${tokenA.publicKey.toBase58()} ${tokenAAccountToFreeze.toBase58()} -k ${
        process.env.KEYPAIR_DEVNET
      }`
    );
    const acctInfo = await getTokenAccountInfo(
      connection,
      tokenAAccountToFreeze
    );
  ...
  it("it should get canonical token balance", async () => {
    const result = test(
      "get-balance-canonical",
      `-k ${process.env.KEYPAIR_MAINNET} -v 2`
    );
  });

  it("it should get wormhole token balance", async () => {
    const result = test(
      "get-balance-wormhole",
      `-k ${process.env.KEYPAIR_MAINNET}`
    );
  });
  it("it should swap canonical rally for wormhole rally", async () => {
    const result = test(
      "swap-canonical-wormhole",
      `-k ${process.env.KEYPAIR_MAINNET} -a 5 `
    );
  });
  it("it should swap wormhole rally for canonical rally", async () => {
    const result = test(
      "swap-wormhole-canonical",
      `-k ${process.env.KEYPAIR_MAINNET} -a 5 `
    );
  });
  it("it should initialize a tbc", async () => {
    ...
    const result = test(
      "tbc-init",
      `${tokenA.publicKey.toBase58()} ${tokenB.publicKey.toBase58()} ${initialTokenBLiquidity.toNumber()} -k ${
        process.env.KEYPAIR_DEVNET
      } --slope_numerator 1 --slope_denominator 200000000 --init_price_a_numerator 50 --init_price_a_denominator 1`
    );
  ...
  it("it should get a tbc", async () => {
    const result = test(
      "get-tbc",
      `${tbcPubKey} -k ${process.env.KEYPAIR_DEVNET}`
    );
  });
  it("it should estimate swap on tbc", async () => {
    const result = test(
      "tbc-swap-estimate",
      `${tbcPubKey} ${tokenA.publicKey.toBase58()} ${tokenB.publicKey.toBase58()} ${swapInitAmountTokenA.toNumber()} -k ${
        process.env.KEYPAIR_DEVNET
      }`
    );
  });
  ...
  it("it should execute reverse swap on tbc", async () => {
    const result = test(
      "tbc-swap",
      `${tbcPubKey} ${tokenB.publicKey.toBase58()} ${tokenA.publicKey.toBase58()} ${swapInitAmountTokenA.toNumber()} -k ${
        process.env.KEYPAIR_DEVNET
      }`
    );
  });
