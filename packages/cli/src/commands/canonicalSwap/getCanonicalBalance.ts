...
export const getCanonicalBalanceCommand = async (options) => {
  ...
  const canonicalMint = new PublicKey(
    version === "2" ? cswap.canonicalMintV2 : cswap.canonicalMintV3
  );

  //decimals of destination-

  const canv2 = new Token(
    connection,
  ...
  const associatedTokenAcct = await canv2.getOrCreateAssociatedAccountInfo(
    wallet.publicKey
  );
  ...
  console.log(
    `balance = ${canBalance.div(ten.pow(new BN(decimals))).toNumber()} in ${
      associatedTokenAcct.address
    }`
  );
};
