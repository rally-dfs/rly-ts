...
export const getWormholeBalanceCommand = async (options) => {
  ...
  const whv2 = new Token(
    connection,
    new PublicKey(cswap.wormholeMint),
    TOKEN_PROGRAM_ID,
    payer
  );
  const { decimals } = await whv2.getMintInfo();
  const associatedTokenAcct = await whv2.getOrCreateAssociatedAccountInfo(
    wallet.publicKey
  );
  ...
  console.log(
    `balance = ${wormBal.div(ten.pow(new BN(decimals))).toNumber()} in ${
