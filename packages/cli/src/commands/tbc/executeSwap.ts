...
export const executeSwapCommand = async (
  swap,
  token_a,
  token_b,
  amount,
  ...
  const swapSourceInfo = await getTokenAccountInfo(
    connection,
    swapData.tokenAccountA
  );
  const swapDestInfo = await getTokenAccountInfo(
    connection,
    swapData.tokenAccountB
  );

  const swapSourceTokenAccount =
    swapSourceInfo.mint.toBase58() === tokenA.publicKey.toBase58()
      ? swapData.tokenAccountA
      : swapData.tokenAccountB;
  const swapDestinationTokenAccount =
    swapDestInfo.mint.toBase58() === tokenB.publicKey.toBase58()
      ? swapData.tokenAccountB
      : swapData.tokenAccountA;
