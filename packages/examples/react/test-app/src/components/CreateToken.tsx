...
const CreateToken: FC = () => {
  ...
  const [tokenReturnValues, setTokenReturnValues] =
    useState(defaultReturnValues);
      ...
      {tokenReturnValues.tx != null && (
        <Stack spacing={1}>
          <Typography variant="body1" color="text.secondary">
            ...
            <Link
              href={`${EXPLORER_ROOT}/tx/${tokenReturnValues.tx}?cluster=${NETWORK}`}
              target="_blank"
            >
              (view transaction)
            </Link>
          ...
          <Typography variant="body1" color="text.secondary">
            token mint:{" "}
            <Link
              href={`${EXPLORER_ROOT}/address/${tokenReturnValues.tokenMint}?cluster=${NETWORK}`}
              target="_blank"
            >{`${tokenReturnValues.tokenMint}`}</Link>
          </Typography>
        </Stack>
      )}
    </Box>
