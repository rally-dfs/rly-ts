...
const InitTbc: FC = () => {
  ...
  const [tbcResponseValues, setTbcResponsValues] = useState(
    defaultInitTbcRespondValues
  );
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (!wallet.publicKey) {
      ...
      const result = await initializeLinearPriceCurve({
        ...
        connection,
        poolTokenDecimals: 9,
        initialTokenBLiquidity: new BN(
          initialTokenBLiquidity * 10 ** Number(tokenBDecimals)
        ),
      });
  ...
  return (
    <Box component="form" noValidate onSubmit={handleSubmit} sx={{ mt: 4 }}>
      <Typography variant="h6">Initialize Token Bonding Curve</Typography>
      <Typography
        variant="subtitle1"
      ...
      {tbcResponseValues.tx != null && (
        <Stack spacing={1}>
          <Typography variant="body1" color="text.secondary">
            {`swap successfully initialized!`}
            <Link
              href={`${EXPLORER_ROOT}/tx/${tbcResponseValues.tx}?cluster=${NETWORK}`}
              target="_blank"
          ...
          <Typography variant="body1" color="text.secondary">
            swap id:{" "}
            <Link
              href={`${EXPLORER_ROOT}/address/${formValues.tokenSwapInfo}?cluster=${NETWORK}`}
              target="_blank"
            >{`${formValues.tokenSwapInfo}`}</Link>
          </Typography>
        </Stack>
      )}
    </Box>
